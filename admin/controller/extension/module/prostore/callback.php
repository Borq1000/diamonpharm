...
class ControllerExtensionModuleProstoreCallback extends Controller { 
	private $error = array();
  	...
  	private function getList() {
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($callbacks_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($callbacks_total - $this->config->get('config_limit_admin'))) ? $callbacks_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $callbacks_total, ceil($callbacks_total / $this->config->get('config_limit_admin')));
  	...
  	private function getForm() {
		...
		if (isset($callback_info) && $callback_info['store_id']) {
