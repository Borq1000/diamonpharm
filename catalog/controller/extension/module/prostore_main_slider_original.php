		...
		if(isset($setting['slider_image'])){
			...
			foreach ($setting['slider_image'] as  $result) {
				...
				if (is_file(DIR_IMAGE . $result['image'])) { 
					...
					// РЕШЕНИЕ: Используем прямые ссылки на оригинальные изображения
					// Это обойдёт и OpenCart resize, и NitroPack обработку
					
					// Формируем прямой URL к оригинальному изображению
					$image_url = $this->request->server['HTTPS'] ? HTTPS_SERVER : HTTP_SERVER;
					$image_url .= 'image/' . $result['image'];
					
					$image2_url = '';
					if ((!empty($result['image2']) && isset($result['image2'])) && is_file(DIR_IMAGE . $result['image2'])) {
						$image2_url = $this->request->server['HTTPS'] ? HTTPS_SERVER : HTTP_SERVER;
						$image2_url .= 'image/' . $result['image2'];
					}
					...
					$data['banners'][$result['sort_order']][] = array(
						...
						// Используем прямые ссылки на оригиналы
						'image' => $image_url,
						'image2' => $image2_url,
					);
				}
